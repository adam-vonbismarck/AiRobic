...
public class UT2DistributionGenerator {
  ...
  public static HashMap<Integer, Double> intervalsProbability = new HashMap<>() {{
    this.put(2, 0.15);
    this.put(3, 0.5);
    this.put(4, 0.25);
    this.put(5, 0.1);
  ...
  private static Emission produceLowIntensityEmission(int numIntervals, int intervalLength) {
    int restTime = switch (numIntervals) {
      case 4, 5 -> 60;
      default -> 90;
    };
    double total = numIntervals*intervalLength;
    return new Emission(numIntervals + "x" + intervalLength + "', " +
        restTime + "s rest at UT2 pace, rate 20.", total, false, null, 5);
  }
