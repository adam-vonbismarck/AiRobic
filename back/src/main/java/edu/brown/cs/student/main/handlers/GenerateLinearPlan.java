...
/**
 * The GenerateLinearPlan class generates a linear plan given the type of workouts the user wants to do, the
 * start and end date of their plan, the amount of time they have, and the amount of high intensity work they
 * want to do.
 */
public class GenerateLinearPlan {

  /**
   * The generate method takes in the constraints with which the schedule should be built, and builds the schedule
   * using the ScheduleBuilder and MarkovModel classes.
   *
   ...
   * @throws InvalidDistributionException if there are invalid distributions involved when building the model.
   * @throws InvalidScheduleException if there are problems building a schedule given the constraints.
   * @throws NoWorkoutTypeException if the high or low intensity labels are not found.
   * @throws FormatterFailureException if there is an issue reading MarkovModel results into a schedule.
   */
